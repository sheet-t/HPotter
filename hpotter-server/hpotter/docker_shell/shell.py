...
def fake_shell(client_socket, connection, prompt, telnet=False):
    ...
    while command_count < 4:
        ...
        cmd = tables.Requests(
            request_type="Shell", request=command, connection=connection
        )
        write_db(cmd)

        exit_code, output = get_shell_container().exec_run(command, workdir=workdir)
        ...
        if exit_code in (126, 127):
            client_socket.sendall(command.encode("utf-8") + b": command not found\r\n")
        else:
