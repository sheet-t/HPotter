...
class Model:
    def __init__(self, num_layers, hidden_size, vocab, embedding_size):
        ...
        decoded_input = self._process_decoder_input(
            target_data=self.targets, batch_size=tf.to_int32(self.batch_size)
        )  # missing arg?
        ...
        decoded_outputs = self._decoder(
            encoded_state=encoded_state, decoded_embed_input=decoded_embed_input
        )
        ...
        self.cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=logits, labels=self.targets, name="cross_entropy"
        )
        self.batch_loss = tf.identity(
            tf.reduce_mean(self.cross_entropy, axis=1), name="batch_loss"
        )
    ...
    def _encoder(self, encoded_embed_input):
        cells = [
            self._lstm_cell(num_hidden_layers=self.hidden_size)
    ...
    def _decoder(self, encoded_state, decoded_embed_input):
        ...
        helper = tf.contrib.seq2seq.TrainingHelper(
            decoded_embed_input, output_lengths, time_major=False
        )
        cells = [
            self._lstm_cell(num_hidden_layers=self.hidden_size)
            for _ in range(self.num_layers)
    ...
    def _optimizer(self, loss):
        ...
        optimizer = tf.contrib.layers.optimize_loss(
            ...
            learning_rate_decay_fn=lambda learning_rate, global_step: _learning_rate_decay_fn(
    ...
    def _process_decoder_input(self, target_data, batch_size):
        ...
        decoded_input = tf.concat(
            [tf.fill([batch_size, 1], self.vocab.vocab["<GO>"]), ending], 1
        )
        return decoded_input

    def _lstm_cell(self, num_hidden_layers):
        cell = tf.contrib.rnn.LSTMCell(
            num_hidden_layers, initializer=tf.contrib.layers.xavier_initializer()
        )
    ...
    def _weight_and_bias(self, output_size):
        weight = tf.Variable(
            tf.truncated_normal([self.hidden_size, output_size], stddev=0.01)
        )
