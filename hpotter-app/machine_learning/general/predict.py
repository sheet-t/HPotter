...
def predict(json_file, num_samples, path):
    with open(json_file, "r") as json_handle, open(
        path + "/samples.txt", "a+"
    ) as text_handle:
        ...
        try:
            ...
            raise Exception(
                "Key Error, Expecting JSON in the Following Format: "
                + json.dumps(
                    {
                        "data": {
                            "requests": [
                                {
                                    "id": "id_goes_here",
                                    "request": "data_goes_here",
                                    "requestType": "Web/SQL/Shell",
                                }
                            ]
                        }
                    },
    ...
    predictor = Predictor(
        checkpoint_path="machine_learning/http_commands/checkpoints/",
        std_factor=25.0,
        vocab=Vocabulary(),
    )
    predicted_data = Data(path="machine_learning/http_commands/data/samples.txt")
    predicted_generator = predicted_data.predict_gen()
    anomalous_predictions, anomaly_losses = predictor.predict(
        data_generator=predicted_generator
    )
